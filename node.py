...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for i in possible_splits:
            left_positive = (y[:i] == 1).sum()
            left_negative = (y[:i] == 0).sum()
            right_positive = (y[i:] == 1).sum()
            right_negative = (y[i:] == 0).sum()
            left = left_positive + left_negative
            right = right_positive + right_negative
            gini_left = 1 - pow(left_positive / left, 2) - pow(left_negative / left, 2)
            gini_right = 1 - pow(right_positive / right, 2) - pow(right_negative / right, 2)
